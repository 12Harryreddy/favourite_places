...
class UserLocation extends StatefulWidget {
  const UserLocation({super.key, required this.onPickLocation});
...
class _UserLocationState extends State<UserLocation> {
  ...
  void _savePlace(double lat, double lng) async {
    final url = Uri.parse(
        'https://maps.googleapis.com/maps/api/geocode/json?latlng=$lat,$lng&key=AIzaSyDtvFfj4_h3nXOV2-PHsZ7ROS2W9OZ0XhM');
  ...
  void _onSavePlace() async {
    final response = await Navigator.of(context).push<LatLng>(
      MaterialPageRoute(
    ...
    _savePlace(response.latitude, response.longitude);
  }
